...
public abstract class Entity implements ICommandSender
{
    ...
    public void func_191958_b(float p_191958_1_, float p_191958_2_, float p_191958_3_, float p_191958_4_)
    {
        ...
        EventStrafe strafe2 = new EventStrafe(p_191958_1_, p_191958_3_, p_191958_4_, this.rotationYaw);

        EventManager.instance.call(strafe2);

        p_191958_1_ = strafe2.getStrafe();
        p_191958_4_ = strafe2.getFriction();
        p_191958_3_ = strafe2.getForward();

        float f = p_191958_1_ * p_191958_1_ + p_191958_2_ * p_191958_2_ + p_191958_3_ * p_191958_3_;

        if (SilentRotation.getTargetRotation() != null) {
            switch (SilentRotation.getCategory()) {
                case Silent -> {
                    float yawFix = SilentRotation.getTargetRotation().getX();
                    float strafe = strafe2.getStrafe();
                    float forward = strafe2.getForward();
                    float friction = strafe2.getFriction();
                    float factor = strafe * strafe + forward * forward;

                    int angleDiff = (int) ( (double) (MathHelper.wrapDegrees(this.rotationYaw - yawFix - 22.5f - 135.0f) + 180.0) / ((double) 45.0));
                    float calcYaw = SilentRotation.getCategory() == MoveCategory.Silent ? yawFix + 45.0f * angleDiff : yawFix;

                    float calcMoveDir = (float) Math.max(Math.abs(strafe), Math.abs(forward));
                    ...
                    if (factor >= 1.0E-4F) {
                        ...
                        this.motionX += (float) strafe * yawCos - forward * yawSin;
                        this.motionY += (double) p_191958_2_;
                        this.motionZ += (float) forward * yawCos + strafe * yawSin;
                    }
                }
                case Strict -> {
                    if (f >= 1.0E-4F) {
                        ...
                        float f1 = MathHelper.sin(strafe2.getYaw() * 0.017453292F);
                        float f2 = MathHelper.cos(strafe2.getYaw() * 0.017453292F);
    ...
    public double getDistanceSq(Entity pos)
    {
        return pos.getDistanceSq(this.posX, this.posY, this.posZ);
    }
    ...
    public double getDistance(Entity pos)
    {
    ...
    public RayTraceResult rayTraceCustom(double blockReachDistance, float yaw, float pitch, float partialTicks) {
        ...
        final Vec3d vec32 = vec3.add(vec31.xCoord * blockReachDistance, vec31.yCoord * blockReachDistance, vec31.zCoord * blockReachDistance);

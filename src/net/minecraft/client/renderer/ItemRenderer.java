...
public class ItemRenderer
{
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        ...
        if (flag1)
        {
            ...
            if (!Reflector.ForgeHooksClient_renderSpecificFirstPersonHand.exists() || !Reflector.callBoolean(Reflector.ForgeHooksClient_renderSpecificFirstPersonHand, EnumHand.OFF_HAND, partialTicks, f1, f4, f6, this.itemStackOffHand))
            {
                ...
                if (Animation.instance.isEnable()) {
                    if (this.mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemSword && this.mc.gameSettings.keyBindUseItem.isKeyDown() || (KillAura.INSTANCE.isEnable() && KillAura.INSTANCE.isBlock)) {
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(p_187457_4_)) {
            ...
            if (p_187457_6_.func_190926_b()) {
                ...
                if (p_187457_1_.isHandActive() && p_187457_1_.getItemInUseCount() > 0 && p_187457_1_.getActiveHand() == p_187457_4_) {
                    ...
                    float f = 1.0F - (this.prevEquippedProgressMainHand + (this.equippedProgressMainHand - this.prevEquippedProgressMainHand) * p_187457_2_);
                    float f1 = p_187457_1_.getSwingProgress(p_187457_2_);
                    ...
                    if ((this.mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemSword && this.mc.gameSettings.keyBindUseItem.isKeyDown() || (KillAura.INSTANCE.isEnable() && KillAura.INSTANCE.isBlock)) && Animation.instance.isEnable()) {
                        ...
                        f = -0.4F * MathHelper.sin(MathHelper.sqrt(p_187457_5_) * (float)Math.PI);
                        f1 = 0.2F * MathHelper.sin(MathHelper.sqrt(p_187457_5_) * ((float)Math.PI * 2F));
    ...
    private void renderFireInFirstPerson()
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            float f8 = 0.5F;
            float f9 = -0.5F;
            GlStateManager.translate((float)(-(i * 2 - 1)) * 0.24F, -0.3F + WorldManager.instance.flameHigh.getValue().floatValue(), 0.0F);
    ...
    public void updateEquippedItem() {
        ...
        EntityPlayerSP entityplayersp = this.mc.player;
        ItemStack itemstack = SpoofSlotUtils.getStack();
        ...
        if (entityplayersp.isRowingBoat()) {
            ...
            {
                boolean flag = Reflector.callBoolean(Reflector.ForgeHooksClient_shouldCauseReequipAnimation, this.itemStackMainHand, itemstack, SpoofSlotUtils.getSlot());
